devil-roundend-name = Диявол
objective-issuer-devil = Диявол

roundend-prepend-devil-contracts = Хтось уклав {$number} успішних контрактів.
roundend-prepend-devil-contracts-named = {$name} уклав(ла) {$number} успішних контрактів.

devil-component-examined = [color=darkred]Очі {CAPITALIZE(POSS-ADJ($target))} ледь світяться червоним.[/color]
condemned-component-examined = [color=chartreuse]Очі {CAPITALIZE($target)} порожні та бездушні.[/color]

devil-role-greeting = Ви диявол з глибин пекла,
  що заволодів тілом {$playerName}
  Розтлівайте смертних і збирайте душі за допомогою контрактів.
  Не у ваших інтересах виконувати завдання, не пов'язані з вашими угодами, вони нижчі за вашу гідність.
  Ваше справжнє ім'я — {$trueName}. Намагайтеся тримати його в таємниці, добре?

devil-true-name-heard = Наказ {$speaker}, що вимовляє справжнє ім'я {$target}, змушує його зупинитися.
devil-true-name-heard-chaplain = Святі слова {$speaker}, що наказують їхнє справжнє ім'я, випалюють саму душу {$target}.

objective-condition-contract-title = Збір Душ
objective-condition-contract-description = Зберіть якомога більше душ.

hand-shake-prompt-verb = Запропонувати рукостискання {$target}
hand-shake-accept-verb = Потиснути руку {$target}

handshake-success = Ви відчуваєте себе трохи легше, потиснувши руку {$user}.
handshake-fail = Ви тиснете руку {POSS-ADJ($user)}, але нічого не відбувається!

handshake-offer-popup = {$user} простягає вам свою руку з хитрою посмішкою на обличчі.
handshake-offer-popup-self = Ви простягаєте руку {$target} з хитрою посмішкою на обличчі.

weaktoholy-component-bible-sizzle = Шкіра {$target} обпалюється {$item}!
devil-banish-begin = {$user} починає проводити екзорцизм над {$target}!
devil-exorcised = Земля тріскається і розколюється під {$target}, коли їх силою повертають до пекла!

invalid-possession-target = Ви не можете вселитися в цю ціль!

devil-power-level-increase-weak = З кінчиків ваших пальців спалахують іскри полум'я. Ваш зв'язок з пеклом зміцнився.
devil-power-level-increase-moderate = Ви відчуваєте приплив сили. Ваш зв'язок з пеклом зміцнився.
devil-power-level-increase-powerful = Ви майже виконали свої завдання. Ще кілька душ.

objective-condition-weight-title = Укладайте вигідні угоди.
objective-condition-weight-description = Переконайтеся, що угоди приносять вам якомога більше користі.