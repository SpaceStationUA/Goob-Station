...
/// <summary>
/// System for handling the weapon recall ability from the recall implant.
/// Based on SharedItemRecallSystem.
/// </summary>
public abstract partial class SharedWeaponRecallSystem : EntitySystem
{
    ...
    private void OnMapInit(Entity<WeaponRecallComponent> ent, ref MapInitEvent args)
    {
    ...
    private void OnWeaponRecallActionUse(Entity<WeaponRecallComponent> ent, ref OnWeaponRecallActionEvent args)
    {
        if (ent.Comp.MarkedEntity == null)
        {
            ...
            if (markItem == null)
            {
                _popups.PopupClient(Loc.GetString("weapon-recall-item-mark-empty"), args.Performer, args.Performer);
                return;
            }

            if (HasComp<WeaponRecallMarkerComponent>(markItem))
            {
                _popups.PopupClient(Loc.GetString("weapon-recall-item-already-marked", ("item", markItem)), args.Performer, args.Performer);
                return;
            }

            // Only allow items with the melee weapon component which, conveniently, includes guns! Also allows random other stuff like tools, but I think that's fine
            if (!HasComp<MeleeWeaponComponent>(markItem))
            {
                _popups.PopupClient(Loc.GetString("weapon-recall-non-weapon"), args.Performer, args.Performer);
                return;
            }
    ...
    private void RecallItem(Entity<WeaponRecallMarkerComponent?> ent)
    {
        ...
        _popups.PopupPredicted(Loc.GetString("weapon-recall-item-summon-self", ("item", ent)),
                               Loc.GetString("weapon-recall-item-summon-others", ("item", ent), ("name", Identity.Entity(user, EntityManager))),
                               user, user);
        _popups.PopupPredictedCoordinates(Loc.GetString("weapon-recall-item-disappear", ("item", ent)), Transform(ent).Coordinates, user);

        _hands.TryForcePickupAnyHand(user, ent);
    }

    private void OnRecallMarkerShutdown(Entity<WeaponRecallMarkerComponent> ent, ref ComponentShutdown args)
    {
        TryUnmarkItem(ent);
    }
