...
public sealed class AlertLevelSystem : EntitySystem
{
    ...
    [Dependency] private readonly PirateAlertLevelAudioSystem _pirateAlertAudio = default!; // Pirate: announcement audio settings
        ...
        bool locked = false, MetaDataComponent? dataComponent = null, AlertLevelComponent? component = null)
    {
        ...
        if (playSound)
        {
            if (detail.Sound != null)
            {
                ...
                _pirateAlertAudio.Play(level, detail.Sound, filter); // Pirate: announcement audio settings
            }
            else
            {
                playDefault = true;
