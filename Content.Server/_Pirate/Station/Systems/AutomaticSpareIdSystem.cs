using System.Linq;
using Content.Server._Pirate.Cabinet;
using Content.Server._Pirate.Station.Components;
using Content.Server._Pirate.Station.Events;
...
namespace Content.Server._Pirate.Station.Systems;
...
public sealed class AutomaticSpareIdSystem : EntitySystem
{
    ...
    public override void Initialize()
    {
        ...
        Subs.CVar(_cfg, CCVars.SpareIdAutoUnlock, value => _autoUnlock = value, true);
        Subs.CVar(_cfg, CCVars.SpareIdAlertDelay, value => _alertDelay = value, true);
        Subs.CVar(_cfg, CCVars.SpareIdUnlockDelay, value => _unlockDelay = value, true);
    }
    ...
    /// <param name="newSpareIdAccess">The access to give to the spare ID cabinet. If not specified or null, defaults to command.</param>
    /// <param name="unlockMessageLocId">The message displayed upon unlocking. If not specified or null, defaults to the component's unlock message.</param>
    private void MoveToUnlocked(Entity<AutomaticSpareIdComponent> ent, ProtoId<AccessLevelPrototype>? newSpareIdAccess = null, LocId? unlockMessageLocId = null)
    {
        ...
        while (query.MoveNext(out var uid, out _, out var accessReader))
        {
            ...
            if (!newSpareIdAccess.HasValue)
                newSpareIdAccess = ent.Comp.GrantAccessToCommand;
        ...
        if (!unlockMessageLocId.HasValue)
            unlockMessageLocId = ent.Comp.UnlockedMessage;
