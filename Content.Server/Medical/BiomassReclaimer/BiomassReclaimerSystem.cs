...
namespace Content.Server.Medical.BiomassReclaimer
{
    public sealed class BiomassReclaimerSystem : EntitySystem
    {
        ...
        private void OnClimbedOn(Entity<BiomassReclaimerComponent> reclaimer, ref ClimbedOnEvent args)
        {
            ...
            _adminLogger.Add(LogType.Action, LogImpact.High, $"{ToPrettyString(args.Instigator):player} використав утилізатор біомаси, щоб розщепити {ToPrettyString(args.Climber):target} в {ToPrettyString(reclaimer):reclaimer}");
        ...
        private void OnDoAfter(Entity<BiomassReclaimerComponent> reclaimer, ref ReclaimerDoAfterEvent args)
        {
            ...
            _adminLogger.Add(LogType.Action, LogImpact.High, $"{ToPrettyString(args.Args.User):player} використав утилізатор біомаси, щоб розщепити {ToPrettyString(args.Args.Target.Value):target} в {ToPrettyString(reclaimer):reclaimer}");
