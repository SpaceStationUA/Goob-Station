...
namespace Content.IntegrationTests.Tests
{
    ...
    public sealed class EntityTest
    {
        ...
        public async Task SpawnAndDeleteEntityCountTest()
        {
            ...
            var persistentServerEntities = new HashSet<EntityUid>();
            var persistentClientEntities = new HashSet<EntityUid>();
            void AddEntities(IEntityManager entMan, HashSet<EntityUid> entities, bool persistent = false)
            {
                var audioQuery = entMan.GetEntityQuery<AudioComponent>();
                var timedDespawnQuery = entMan.GetEntityQuery<TimedDespawnComponent>();
                foreach (var e in entMan.GetEntities())
                {
                    if (audioQuery.HasComp(e) || persistent && timedDespawnQuery.HasComp(e))
                        continue;
            ...
            await Assert.MultipleAsync(async () =>
            {
                foreach (var protoId in protoIds)
                {
                    ...
                    persistentServerEntities.Clear();
                    AddEntities(server.EntMan, persistentServerEntities, persistent: true);
                    persistentClientEntities.Clear();
                    AddEntities(client.EntMan, persistentClientEntities, persistent: true);
                    // </Trauma>
