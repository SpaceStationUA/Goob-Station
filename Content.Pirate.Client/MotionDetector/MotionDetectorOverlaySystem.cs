...
public sealed class MotionDetectorOverlaySystem : EntitySystem
{
        ...
        Texture texture)
    {
        ...
        var playerCoordinates = transform.GetMapCoordinates(player);
        ...
        foreach (var held in hands.EnumerateHeld(player))
        {
            ...
            if (activeDetector != held || lastScan != detector.LastScan)
            {
                ...
                foreach (var blip in detector.Blips)
                {
                    ...
                    Cap(ref difference.X, viewportWidth);
                    Cap(ref difference.Y, viewportHeight);
                    blips.Add(difference);
                }
            }

            foreach (var blip in blips)
                handle.DrawTexture(texture, playerCoordinates.Position + blip);
