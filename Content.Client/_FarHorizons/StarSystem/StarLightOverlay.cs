...
/// <summary>
/// Ported from Monolith (#4494): directional starlight for star system maps, with wall
/// shadows so the far side of a grid stays dark.
/// </summary>
public sealed class StarLightOverlay : Overlay
{
    ...
    protected override void Draw(in OverlayDrawArgs args)
    {
        ...
        // Far Horizons: our star system maps position the system via StarOffset (the station
        // anchors the layout) — the star's world position is Star.Position + StarOffset.
        var map = _entMan.GetComponent<StarSystemMapComponent>(args.MapUid);
        var star = map.StarSystem!.Star;
        var starWorldPos = star.Position + map.StarOffset;
        ...
        var power = MathF.Pow(MathF.Max(0f, star.Luminosity), 0.4f);
        power = power / (1f + power) * comp.Intensity;

        _shader ??= _protoMan.Index(StarLightShader).InstanceUnique();
        _shader.SetParameter("starPos", starWorldPos);
        ...
        worldHandle.RenderInRenderTarget(target,
            () =>
            {
                ...
                DrawGrids(worldHandle, invMatrix, mapId, bounds, eyePos, comp, star, starWorldPos);
        ...
        Vector2 starWorldPos)
    {
        ...
        foreach (var grid in _grids)
        {
            ...
            var toStar = starWorldPos - gridPos;
    ...
    {
        // Monolith checks a ContentTileDefinition.Transparent flag their fork adds; our content
        // has no opaque tiles (all non-empty tiles are walkable, light-receiving floors), so
        // only empty tiles see the star through a grid.
        return tile.IsEmpty;
    }
