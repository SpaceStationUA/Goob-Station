...
public abstract partial class SharedNuclearReactorSystem : EntitySystem
{
    ...
    public void UpdateUI(Entity<NuclearReactorComponent> ent, EntityUid? uiTarget = null)
    {
        var target = uiTarget ?? ent.Owner;

        // client doesnt have the data to predict the detailed slot state
        if (_net.IsClient || !_ui.IsUiOpen(target, NuclearReactorUiKey.Key))
            return;

        if (ent.Comp.Melted)
        {
            _ui.CloseUi(target, NuclearReactorUiKey.Key);
            return;
        }
